...
export const CollectionArchive: React.FC<Props> = props => {
  ...
  return (
    <div
      className={`bg-logo-200  ${[classes.collectionArchive, className].filter(Boolean).join(' ')}`}
    >
          ...
          <div className={`${classes.grid} justify-center`}>
            {results.docs?.map((result, index) => {
              if (typeof result === 'object' && result !== null) {
                return (
                  <div className={`mx-auto lg:mx-0 ${classes.column}`} key={index}>
                    <Card doc={result} className="mx-auto" relationTo={relationTo} showCategories />
                  </div>
                )
              }
