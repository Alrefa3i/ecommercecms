...
import { SAMPLE_PRODUCTS } from '../../_graphql/products'
...
const fetchDocsProducts = (slug: string) => {
  if (slug === 'home' || slug === '') {
    return fetchDocs<Product>('products', SAMPLE_PRODUCTS)
  }
}
...
export default async function Page({ params: { slug = 'home' } }) {
  ...
  try {
    ;[page, products] = await Promise.all([
      fetchDoc<PageType>({
        collection: 'pages',
        slug,
        draft: isDraftMode,
      }),
      fetchDocsProducts(slug),
    ])
  } catch (error) {
    console.error(error)
  }
